...
from equivariant_diffusion.egnn_new import EGNN, GNN, coord2diff
...
class EGNNDynamics(nn.Module):
    def __init__(self, atom_nf, residue_nf, n_dims, n_classes=7,
                 joint_nf=16, hidden_nf=64, device='cpu',
                 ...
                 context_mode=None, context_node_nf=0):
        super().__init__()
        self.mode = mode
        ...
        if context_mode is not None:
            self.context_encoder = nn.Sequential(
                ...
                act_fn,
                nn.Linear(2 * n_classes, context_node_nf)
            )
        ...
        if mode == 'egnn_dynamics':
            self.egnn = EGNN(
                ...
                context_node_nf=context_node_nf, context_mode=context_mode,
            )
    ...
    def forward(self, xh_atoms, xh_residues, t, mask_atoms, mask_residues, context=None):
        ...
        if context is not None and self.context_encoder is not None:
            cemb = self.context_encoder(context)
...
class EGNNClassifier(EGNN):
    def __init__(self, n_classes=7, n_fc=3, edge_cutoff_pocket=None, **kwargs):
        
        assert 'context_mode' not in kwargs or kwargs['context_mode'] is None
        ...
        fcs = []
        for _ in range(n_fc):
            fcs.append(nn.Linear(hidden_nf, hidden_nf))
            fcs.append(act_fn)
        fcs.append(nn.Linear(hidden_nf, n_classes))
    ...
    def prepare_input_data(self, x, h, mask):
        ...
        h_order = utils.arange_by_partition(mask)
        h = torch.cat([h, h_order], dim=1)

        # get edges of a complete graph
        adj = mask[:, None] == mask[None, :]

        if self.edge_cutoff_p is not None:
            adj = adj & (torch.cdist(x, x) <= self.edge_cutoff_p)
    ...
    def forward(self, h, x, edge_index, node_mask=None, edge_mask=None, update_coords_mask=None,
        ...
        # graph pooling
        h_pool = scatter_mean(h, batch_mask, dim=0)
        logits = self.fcs(h_pool)
        
        return x, h, logits, h_pool
