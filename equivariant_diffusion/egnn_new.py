...
class EGNN(nn.Module):
    def __init__(self, in_node_nf, in_edge_nf, hidden_nf, device='cpu', act_fn=nn.SiLU(), n_layers=3, attention=False,
                 ...
                 context_node_nf=0, context_mode=None):
        super().__init__()
        ...
        assert context_mode is None or context_mode in ['add', 'concat']
        self.context_mode = context_mode
        self.context_node_nf = context_node_nf
        if context_mode == 'add':
            self.context_embedding = nn.Linear(context_node_nf, hidden_nf)
        ...
        for i in range(0, n_layers):
            self.add_module("e_block_%d" % i, EquivariantBlock(hidden_nf+context_node_nf if self.context_mode=='concat' else hidden_nf, 
    ...
    def forward(self, h, x, edge_index, node_mask=None, edge_mask=None, update_coords_mask=None,
                batch_mask=None, edge_attr=None, cemb=None):
        # Edit Emiel: Remove velocity as input
        ...
        for i in range(0, self.n_layers):
            if self.context_mode == 'add':
                h_cemb = self.context_embedding(cemb)
                h += h_cemb
            ...
            if self.context_mode == 'concat':
                h[:, -self.context_node_nf:] = cemb

        if self.context_mode == 'concat':
            h = h[:, :-self.context_node_nf]
