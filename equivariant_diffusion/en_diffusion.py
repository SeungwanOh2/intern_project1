...
class EnVariationalDiffusion(nn.Module):
    ...
    def __init__(
            ...
            clf_guidance=False, clf_grad_scale=0., clf_model=None,
            cf_guidance=False, cf_threshold=0., cf_strength=0.):
        super().__init__()
        ...
        assert not (clf_guidance and cf_guidance)
        
        self.clf_guidance = clf_guidance
        self.clf_grad_scale = clf_grad_scale
        self.clf_model = clf_model
            
        self.cf_guidance = cf_guidance
        self.cf_threshold = min(max(0, cf_threshold), 1)
        self.cf_strength = max(0, cf_strength)
        
        self.class_cond = cf_guidance or clf_guidance
    ...
    def sample_p_xh_given_z0(self, z0_lig, z0_pocket, lig_mask, pocket_mask,
                             batch_size, fix_noise=False, context=None):
        """Samples x ~ p(x|z0)."""
...
#         if self.cf_guidance and self.cf_strength > 0:
#             n_lig_nodes = z0_lig.shape[0]
#             n_pocket_nodes = z0_pocket.shape[0]
#             context_all = torch.cat([context[:n_lig_nodes], torch.zeros_like(context[:n_lig_nodes]),
#                                      context[n_lig_nodes:n_pocket_nodes], torch.zeros_like(context[n_lig_nodes:n_pocket_nodes])], dim=0)
#             expanded_ligand_mask = torch.cat([ligand_mask, ligand_mask+batch_size])
#             expanded_pocket_mask = torch.cat([pocket_mask, pocket_mask+batch_size])
#             net_out_lig, net_out_pocket_all = self.dynamics(
#                 z0_lig.repeat(2,1), z0_pocket.repeat(2,1), t_zeros.repeat(2,1),
#                 expanded_ligand_mask, expanded_pocket_mask, context_all)
            
#             net_out_lig_cond, net_out_lig_uncond = torch.split(net_out_lig_all, n_lig_nodes, dim=0)
#             net_out_lig = net_out_lig_uncond - self.cf_strength * (net_out_lig - net_out_lig_uncond) 
            
#             net_out_pocket_cond, net_out_pocket_uncond = torch.split(net_out_pocket_all, n_pocket_nodes, dim=0)
#             net_out_pocket = net_out_pocket_uncond - self.cf_strength * (net_out_pocket - net_out_pocket_uncond) 
#         else:
#             net_out_lig, net_out_pocket = self.dynamics(
#             z0_lig, z0_pocket, t_zeros, lig_mask, pocket_mask, context)
    ...
    def sample_noised(self, ligand, pocket, timestep=None):
        """
        Sample noised data
        """
        ...
        if timestep is None:
            ...
            t = timestep * torch.ones(ligand['size'].size(0), 1, device=ligand['x'].device).float()
            s = (t * self.T - 1) / self.T
        ...
        return z_t_lig, z_t_pocket, eps_t_lig, eps_t_pocket, xh_lig, xh_pocket, s, gamma_s, t, gamma_t
    ...
    def forward(self, ligand, pocket, return_info=False):
        ...
        z_t_lig, z_t_pocket, eps_t_lig, eps_t_pocket, xh_lig, xh_pocket, s, gamma_s, t, gamma_t = \
            self.sample_noised(ligand, pocket)
        ...
        context = None
        if self.class_cond:
            pocket_context = utils.generate_context_vector(
                pocket['labels'], pocket['n_classes'], pocket['size'], self.cf_threshold
            ) 
            context = torch.cat([
                torch.zeros(ligand['one_hot'].shape[0], pocket['n_classes'], device=ligand['one_hot'].device),
                pocket_context
            ])

        # Neural net prediction.
        net_out_lig, net_out_pocket = self.dynamics(
            z_t_lig, z_t_pocket, t, ligand['mask'], pocket['mask'], context)
        ...
        if self.training:
            ...
            net_out_0_lig, net_out_0_pocket = self.dynamics(
                z_0_lig, z_0_pocket, t_zeros, ligand['mask'], pocket['mask'], context)
            ...
            # self.clf_model.compute_grad(z_t_pocket, pocket['mask'], pocket['labels'])
    ...
    def sample_p_zs_given_zt(self, s, t, zt_lig, zt_pocket, ligand_mask,
                             pocket_mask, fix_noise=False, labels=None, context=None):
        ...
        # classifier-free guidance sampling
        if self.cf_guidance and self.cf_strength > 0:
            n_lig_nodes = zt_lig.shape[0]
            n_pocket_nodes = zt_pocket.shape[0]
            batch_size = len(torch.unique(ligand_mask))
            context_all = torch.cat([context[:n_lig_nodes], torch.zeros_like(context[:n_lig_nodes]),
                                     context[n_lig_nodes:], torch.zeros_like(context[n_lig_nodes:])], dim=0)
            expanded_ligand_mask = torch.cat([ligand_mask, ligand_mask+batch_size])
            expanded_pocket_mask = torch.cat([pocket_mask, pocket_mask+batch_size])
            eps_t_lig_all, eps_t_pocket_all = self.dynamics(
                zt_lig.repeat(2,1), zt_pocket.repeat(2,1), t.repeat(2,1),
                expanded_ligand_mask, expanded_pocket_mask, context_all)

            eps_t_lig_cond, eps_t_lig_uncond = torch.split(eps_t_lig_all, n_lig_nodes, dim=0)
            eps_t_lig = eps_t_lig_uncond - self.cf_strength * (eps_t_lig_cond - eps_t_lig_uncond) 

            eps_t_pocket_cond, eps_t_pocket_uncond = torch.split(eps_t_pocket_all, n_pocket_nodes, dim=0)
            eps_t_pocket = eps_t_pocket_uncond - self.cf_strength * (eps_t_pocket_cond - eps_t_pocket_uncond) 
        else:
            # Neural net prediction.
            eps_t_lig, eps_t_pocket = self.dynamics(
                zt_lig, zt_pocket, t, ligand_mask, pocket_mask, context)
        ...
        if self.clf_guidance:
            z_grad_pocket = self.clf_model.compute_grad(zt_pocket, pocket_mask, labels)
            mu_pocket += z_grad_pocket * sigma[pocket_mask] * self.clf_grad_scale
    ...
    def sample(self, n_samples, num_nodes_lig, num_nodes_pocket, pocket=None,
        ...
        if self.class_cond:
            ...
            pocket_context = utils.generate_context_vector(
                pocket['labels'], pocket['n_classes'], num_nodes_pocket.to(device), self.cf_threshold
            ) 
            context = torch.cat([
                torch.zeros(torch.sum(num_nodes_lig), pocket['n_classes'], device=device),
                pocket_context
            ])
        ...
        for s in reversed(range(0, timesteps)):
            ...
            z_lig, z_pocket = self.sample_p_zs_given_zt(
                ...
                labels=pocket['labels'] if self.class_cond else None, context=context)
        ...
        x_lig, h_lig, x_pocket, h_pocket = self.sample_p_xh_given_z0(
            z_lig, z_pocket, lig_mask, pocket_mask, n_samples, context=context)
    ...
    def inpaint(self, ligand, pocket, lig_fixed, pocket_fixed, resamplings=1,
        ...
        if self.class_cond:
            ...
            context = torch.cat([
                torch.zeros(ligand['one_hot'].shape[0], pocket['n_classes'], device=z_pocket.device),
                pocket_context
            ])
        ...
        for i, n_denoise_steps in enumerate(schedule):
            for j in range(n_denoise_steps):
                ...
                if j == n_denoise_steps - 1 and i < len(schedule) - 1:
                    ...
                    # assume that noising process is independent to context (this may be not true)
        ...
        # Finally sample p(x, h | z_0).
        x_lig, h_lig, x_pocket, h_pocket = self.sample_p_xh_given_z0(
            z_lig, z_pocket, ligand['mask'], pocket['mask'], n_samples, context=context)
