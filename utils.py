...
def generate_context_vector(labels, n_classes, n_nodes, threshold=0.):
    
    if threshold > 0:
        uncond_idc = torch.rand(len(labels), device=labels.device) < threshold
        labels[uncond_idc] = -1
    
    labels = torch.repeat_interleave(labels, n_nodes)
    
    cond_mask = torch.where(labels >= 0)[0]
    context = torch.zeros(torch.sum(n_nodes), n_classes, device=labels.device)
    
    context[cond_mask] = F.one_hot(labels[cond_mask], num_classes=n_classes).to(dtype=torch.float32)
    
    return context


def arange_by_partition(mask):
    sect_idc = torch.where(torch.diff(mask))[0]
    y = torch.cat([sect_idc+1, torch.tensor([len(mask)], device=mask.device)]) - torch.cat([torch.zeros(1, device=mask.device), sect_idc+1]) 
    x = torch.arange(len(mask), device=mask.device) + 1 - torch.cat([torch.zeros(1, device=mask.device), sect_idc+1]).repeat_interleave(y.int())
    return x.view((-1, 1))
